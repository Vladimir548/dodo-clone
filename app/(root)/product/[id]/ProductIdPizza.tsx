...
export default function ProductIdPizza({modalClass,data,isPending}:IProductId) {
        ...
        <Container className={`text-white w-full grid grid-cols-2  justify-between gap-x-4 pt-8 ${modalClass && 'pt-0'} `}>
            <div className={' relative  flex justify-center items-center  '}>
                <ImageProduct isLoading={isPending} alt={data?.name}
                              src={data?.productVariant?.find(val => val.doughName === typeDough)?.image}
                              sizes={sizesByCategory?.map((val,index) =>({[val.id]:arrSizeImg[index]}))} size={selectSize}/>
            </div>
            <div className={'flex flex-col gap-y-3  '}>
                ...
                <div className={'flex gap-x-2 text-black/70 dark:text-white/70 '}>
                    <span>{sizesByCategory?.find(val => val.id === selectSize)?.value}</span>
                ...
                <div className={` ${modalClass && ' pr-3 h-[400px]  scrollbar overflow-auto'}  flex flex-col gap-y-2`}>
                    ...
                    <div
                        ...
                        {sizesByCategory?.map(val => {
                            return (
                                <Button
                                    disabled={ProductService.isSizeTypeDough(data,typeDough,val.id)}
                                    variant={"outline"} onClick={() => setSelectSize(val.id)}
                                    className={`w-full  border-2 ${selectSize === val.id && 'bg-primary text-white'} border-primary  rounded-lg hover:border-primary font-bold`}
                                    key={val.id}>{val.value}</Button>
                            )
                        })}
                    </div>
