    ...
    return (
        <>
            <li className={'relative flex items-start gap-x-2'} key={item.id}>
                <div>
                    <Image width={60} height={60} src={`${URL_API}/${item.productVariant?.image}`}
                           alt={item.product.name}/>
                </div>
                <div>
                    <h3 className={'font-bold'}>{item.product.name}</h3>
                    <div className={'flex items-center gap-x-3 dark:text-gray-200 text-black/60 text-sm'}>
                    ...
                    {item.ingredients.length > 0 && (
                        <div className={'dark:text-gray-200 text-black/60 text-xs flex gap-x-1'}>
            ...
            <span className={'flex items-center gap-x-2'}>Цена: <b className={'text-primary'}>{totalPrice} ₽</b></span>
            </div>
