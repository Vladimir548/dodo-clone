...
export default function DialogCart() {
    ...
    if (!data) return  (
        <Button className={'flex items-center gap-x-1 rounded-lg w-[140px]'} variant={"default"}>
            <b className={'w-[40px] line-clamp-1 overflow-hidden'}> 0</b><b>₽</b>
    ...
    return (
        <Drawer direction={"right"}>
            <DrawerTrigger asChild>
                <Button className={'flex items-center gap-x-1 rounded-lg w-[140px]'} variant={"default"}>
                    <b className={'w-[40px] line-clamp-1 overflow-hidden'}>{data?.totalAmount ?? 0}</b><b>₽</b>
                    <span className={'w-[1px] h-full bg-white/30'}></span>
                    <div className={' relative'}>
                        <span className={'flex items-center opacity-100 gap-x-2 duration-300 group-hover:opacity-0'}>
            ...
            <DrawerContent>
                {data?.items?.length > 0 ? (
                    <>
                        <DrawerHeader>
                            <DrawerTitle>В
                                корзине {data?._count?.items} {definitionGoods} </DrawerTitle>
                        </DrawerHeader>
                        ...
                        <DrawerFooter className={'px-5 '}>
                            <div className={' flex flex-col gap-y-3'}>
                                <div className={'flex flex-col gap-y-3 border-b pb-2 border-gray-500 '}>
                                    <p className={'  flex items-center justify-between '}> {data?._count?.items} {definitionGoods}
                                        <b>{data?.amountGoods} ₽</b></p>
                                    <p className={'  flex items-center justify-between '}><span className={'flex items-center gap-x-1'}>Доставка <TooltipDelivery/></span> <b>{priceDelivery}</b>
                                    </p>
                                </div>
                                <p className=" flex items-center justify-between  text-lg">
                                    Сумма заказа
                                    <span className="flex-grow  dotted-line"></span>
                                    <b className="text-nowrap text-primary">{data?.totalAmount} ₽</b>
                                </p>
                            </div>
                            <DrawerClose asChild>
                                <Link href={'/order'}>
                                    <Button className={'w-full py-6 font-bold text-base'}>Перейти к оформлению</Button>
                                </Link>
                            </DrawerClose>
                        </DrawerFooter>
                    </>
                    ...
                    <div className={'flex flex-col justify-center items-center h-full gap-y-3'}>
                        ...
                        <Image quality={100} src={'/box.png'} alt={'box'} width={120} height={120}/>
                        <Title text={'Корзина пуста'}/>
                        <DrawerClose asChild>
                            <Button  className={'flex items-center gap-x-2'}> <ArrowLeft size={20} /> Вернуться назад</Button>
                        </DrawerClose>
                    </div>
                )}
            </DrawerContent>
